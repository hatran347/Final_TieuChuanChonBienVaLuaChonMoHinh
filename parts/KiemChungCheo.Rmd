...
Để hạn chế nhược điểm lãng phí dữ liệu của phương pháp tập kiểm chứng, **Leave-One-Out Cross-Validation (LOOCV)** sử dụng gần như toàn bộ dữ liệu để huấn luyện mô hình ở mỗi lần lặp.
...
Trong đó, $y_i - \hat{y}_i$ là **phần dư (residual)** khi mô hình được khớp trên toàn bộ dữ liệu, còn $h_i$ là **giá trị đòn bẩy (leverage)** của quan sát thứ $i$, được lấy từ đường chéo của **ma trận hat (hat matrix)**.
...
Đây là phương pháp trung hòa, chuẩn mực và được sử dụng rộng rãi nhất trong thực tế hiện nay.
...
(Lưu ý: Nếu cấu hình hệ số $K = n$, mỗi fold sẽ thu hẹp lại chỉ chứa đúng 1 quan sát duy nhất. Quy trình xoay vòng $K$ lần lúc này đồng nhất tuyệt đối với cơ chế hoạt động của LOOCV).
...
- *Sự phụ thuộc vào cấu hình số lượng Fold (*$K$): Hiệu quả của thuật toán bị chi phối trực tiếp bởi việc lựa chọn giá trị $K$. Nếu chọn $K$ quá nhỏ (ví dụ $K=2$), mô hình sẽ rơi vào bẫy thiên vị cao (High Bias) do tập huấn luyện bị thu hẹp đáng kể so với dung lượng mẫu ban đầu.

- *Hạn chế đối với cấu trúc dữ liệu đặc thù:* $K$-Fold Cross-Validation thuần túy giả định các quan sát độc lập và phân phối cùng loại (i.i.d). Do đó, nó sẽ hoạt động kém hiệu quả hoặc làm sai lệch kết quả đối với dữ liệu chuỗi thời gian (Time-series data) do phá vỡ tính phụ thuộc thời gian, hoặc dữ liệu mất cân bằng nghiêm trọng (Imbalanced data) nếu các fold ngẫu nhiên không giữ được tỷ lệ phân phối của các lớp cấu thành.
